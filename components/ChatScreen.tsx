...
import { ChatService, ChatMessage, ChatSession as ChatSessionType } from "../services/chatService";
...
export function ChatScreen({
  ...
  // Initialize chat and timer when component mounts
  useEffect(() => {
    initializeChat();
    return () => cleanup();
  }, [chatSession.id]);

  // Initialize chat and services
    ...
    try {
      ...
      await timerService.current.initializeTimer(
        { 
          uid: 'current-user',
          isPremium,
          dailyFreeTimeUsed: 1200 - dailyFreeTimeRemaining, // Calculate used time
          paidTimeAvailable,
      ...
      const unsubscribeTimer = timerService.current.subscribe((state) => {
        setTimerState(state);
        
        // Check for low time alert
        const totalTime = state.freeTimeLeft + state.paidTimeLeft;
        if (totalTime <= 180 && totalTime > 0 && !isPremium) {
          onLowTimeAlert(state.freeTimeLeft, state.paidTimeLeft);
        }
        
        // Check if timer ended
        if (totalTime <= 0 && !isPremium) {
          onTimerEnd();
        }
      });
      ...
      const unsubscribeMessages = ChatService.subscribeToMessages(
        ...
        (chatMessages: ChatMessage[]) => {
          const formattedMessages: Message[] = chatMessages.map(msg => ({
            id: msg.id,
            text: msg.text,
            sender: msg.senderId === 'current-user' ? 'user' : 
                   msg.senderId === 'system' ? 'system' : 'partner',
      ...
      chatMessageUnsubscribe.current = unsubscribeMessages;
  ...
  const cleanup = () => {
    ...
    if (chatMessageUnsubscribe.current) {
  ...
  const getWelcomeMessage = () => {
    if (chatSession.interlocutor.username === "@SafetalkAI") {
  ...
  const handleCloseChat = () => {
    if (timerState) {
      onCloseChat(timerState.freeTimeLeft, timerState.paidTimeLeft);
    } else {
      onCloseChat(0, 0);
    }
  };
  ...
  const handleChangePartner = () => {
    ...
    ChatService.endChatSession(
      chatSession.id,
      Math.floor((Date.now() - chatSession.startTime) / 1000),
        ...
        <View style={styles.header}>
          ...
          <View style={styles.partnerInfo}>
            <Text style={styles.partnerName}>
              {chatSession.interlocutor.username}
            </Text>
